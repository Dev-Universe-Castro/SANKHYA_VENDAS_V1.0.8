...
export default function PedidoVendaForm({ onSuccess, onCancel }: PedidoVendaFormProps) {
  ...
  const carregarVendedores = () => {
    try {
      ...
      if (cachedVendedores) {
        ...
        console.log('✅ Vendedores carregados do cache:', vendedoresAtivos.length)
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const razaoSocial = dadosAtuaisPedido.RAZAO_SOCIAL || dadosAtuaisPedido.RAZAOSOCIAL
      ...
      if (response.ok) {
        toast.success('Pedido criado com sucesso!', {
          description: `Número: ${result.NUNOTA}`
        })
