...
export default function PedidosTable() {
  ...
  const loadPedidos = async () => {
    try {
      ...
      // Buscar do cache
      const cached = sessionStorage.getItem('cached_pedidos');
      if (cached) {
        try {
          const cachedData = JSON.parse(cached);
          const allPedidos = Array.isArray(cachedData) ? cachedData : (cachedData.pedidos || []);
          ...
          if (allPedidos.length > 0) {
            ...
            console.log(`✅ ${filteredPedidos.length} pedido(s) encontrado(s) no cache`);
          ...
          console.warn('⚠️ Erro ao parsear cache de pedidos');
        }
      }
      ...
      toast({
        title: "Sem dados",
        description: "Nenhum pedido encontrado. Faça login para sincronizar.",
        variant: "default",
      });
  ...
  const handleSearchKeyPress = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      // Limpar o cache quando uma nova busca for realizada com filtros
      if (searchNunota || searchParceiro) {
        sessionStorage.removeItem('cached_pedidos');
      }
      loadPedidos()
    }
  }
  ...
  const handleNovoPedidoSucesso = () => {
    ...
    if (searchNunota || searchParceiro) {
      ...
      // Se não houver filtros, recarregar do cache ou API
      sessionStorage.removeItem('cached_pedidos'); // Limpa cache para forçar recarga da API
      loadPedidos();
    }
  }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="md:hidden border-b p-4">
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-xl font-bold">Pedidos de Venda</h1>
          <Button 
            onClick={() => setShowNovoPedidoModal(true)}
            ...
            <table className="w-full">
              <thead className="sticky top-0 z-10" style={{ backgroundColor: 'rgb(35, 55, 79)' }}>
                <tr>
                  ...
                  <th className="px-3 md:px-6 py-4 text-right text-xs md:text-sm font-semibold text-white uppercase tracking-wider">
                    Valor
                  </th>
                </tr>
              </thead>
                ...
                {loading ? (
                  ...
                  pedidos.map((pedido) => (
                    <tr key={pedido.NUNOTA} className="hover:bg-muted/50 transition-colors">
                      <td className="px-3 md:px-6 py-4 text-xs md:text-sm font-medium text-foreground">{pedido.NUNOTA}</td>
                      <td className="px-3 md:px-6 py-4 text-xs md:text-sm text-foreground">{pedido.PARCEIRO}</td>
                      <td className="px-3 md:px-6 py-4 text-xs md:text-sm text-foreground hidden lg:table-cell">{pedido.VENDEDOR}</td>
                      <td className="px-3 md:px-6 py-4 text-xs md:text-sm text-foreground hidden xl:table-cell">{formatDate(pedido.DTNEG)}</td>
                      <td className="px-3 md:px-6 py-4 text-xs md:text-sm text-right font-semibold text-foreground">{formatCurrency(pedido.VLRNOTA)}</td>
                    </tr>
                  ))
                )}
              </tbody>
