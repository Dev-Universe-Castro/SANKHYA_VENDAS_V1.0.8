...
import { Sparkles, BarChart3, Target, CheckCircle, XCircle, DollarSign } from "lucide-react"
...
export default function DashboardHome() {
  ...
  const loadParceiros = async () => {
    try {
      const response = await fetch('/api/sankhya/parceiros')
      if (!response.ok) throw new Error('Erro ao carregar parceiros')
      const data = await response.json()
      setParceiros(data.slice(0, 100)) // Limitar a 100 parceiros iniciais
    } catch (error) {
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
        <Card className="border-l-4 border-l-blue-500">
          <CardHeader className="flex flex-row items-center justify-between pb-0.5 p-2 md:p-3">
          ...
          <CardContent className="p-2 md:p-3 pt-0">
            <div className="text-lg md:text-xl font-bold">{formatarNumero(dashboardData?.totalLeads || 0)}</div>
        ...
        <Card className="border-l-4 border-l-green-500">
          ...
          <CardContent className="p-2 md:p-3 pt-0">
            <div className="text-lg md:text-xl font-bold">{formatarNumero(dashboardData?.leadsGanhos || 0)}</div>
            <p className="text-[10px] md:text-xs text-green-600">
              {dashboardData?.totalLeads ? ((dashboardData.leadsGanhos / dashboardData.totalLeads) * 100).toFixed(1) : 0}%
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-red-500">
          ...
          <CardContent className="p-2 md:p-3 pt-0">
            <div className="text-lg md:text-xl font-bold">{formatarNumero(dashboardData?.leadsPerdidos || 0)}</div>
            <p className="text-[10px] md:text-xs text-muted-foreground">
              Taxa: {dashboardData?.totalLeads ? ((dashboardData.leadsGanhos / dashboardData.totalLeads) * 100).toFixed(1) : 0}%
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-purple-500">
          ...
          <CardContent className="p-2 md:p-3 pt-0">
            <div className="text-lg md:text-xl font-bold">{formatarMoeda(dashboardData?.valorTotal || 0)}</div>
            <p className="text-[10px] md:text-xs text-muted-foreground">
              Pipeline total
            </p>
          </CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Gráfico de Barras - Leads Ganhos x Perdidos por Dia */}
        <Card>
          <CardHeader>
            <CardTitle>Leads Ganhos x Perdidos por Dia</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        {/* Gráfico de Funil - Quantidade de Leads por Estágio */}
        <Card>
          <CardHeader>
