...
export default function PedidoVendaFromLead({ dadosIniciais, onSuccess, onCancel, onSalvarPedido, isLeadVinculado = true }: PedidoVendaFromLeadProps) {
  ...
  const aplicarConfiguracoesTipoPedido = (tipoPedido: any) => {
    ...
    setPedido(prev => ({
      ...prev,
      CODTIPOPER: String(tipoPedido.CODTIPOPER),
      TIPMOV: tipoPedido.TIPMOV,
      CODTIPVENDA: String(tipoPedido.CODTIPVENDA)
    }))
    ...
    toast.success(`Tipo de pedido "${tipoPedido.NOME}" aplicado`, {
      description: 'Configurações carregadas automaticamente'
    })
  }
  ...
  const carregarVendedores = () => {
    try {
      // Buscar vendedores do cache do prefetch
      const cachedVendedores = sessionStorage.getItem('cached_vendedores')

      if (cachedVendedores) {
        const parsedCache = JSON.parse(cachedVendedores)
        const vendedoresList = Array.isArray(parsedCache) ? parsedCache : (parsedCache.data || [])
        ...
        console.log('✅ Vendedores carregados do cache (PedidoFromLead):', vendedoresAtivos.length)
      }
    } catch (error) {
      console.error('Erro ao carregar vendedores do cache:', error)
      setVendedores([])
    }
  }
  ...
  const carregarTiposNegociacao = async () => {
    try {
      // Tentar buscar do cache primeiro
      const cachedTiposNegociacao = sessionStorage.getItem('cached_tiposNegociacao')
      if (cachedTiposNegociacao) {
        try {
          const data = JSON.parse(cachedTiposNegociacao)
          setTiposNegociacao(Array.isArray(data) ? data : [])
          console.log('✅ Tipos de negociação carregados do cache')
          return
        } catch (e) {
          console.error('Erro ao parsear cache de tipos de negociação:', e)
          sessionStorage.removeItem('cached_tiposNegociacao')
        }
      }
      ...
      const data = await response.json()
      const tiposNegociacaoList = data.tiposNegociacao || []
      setTiposNegociacao(tiposNegociacaoList)

      // Salvar no cache
      if (tiposNegociacaoList.length > 0) {
        sessionStorage.setItem('cached_tiposNegociacao', JSON.stringify(tiposNegociacaoList))
      }
    } catch (error) {
    ...
    try {
      ...
      if (cachedTiposOperacao) {
        try {
          ...
          setTiposOperacao(Array.isArray(data) ? data : [])
          console.log('✅ Tipos de operação carregados do cache')
          return
        } catch (e) {
  ...
  // Função atualizada para carregar tabelas de preço configuradas do cache
  ...
  const buscarParceiros = async (search: string) => {
    ...
    try {
      console.log('🔍 Buscando parceiros no cache para:', search)

      // Buscar APENAS do cache local (vindo do prefetch)
      const cachedParceiros = sessionStorage.getItem('cached_parceiros')

      console.log('📦 Cache bruto encontrado:', cachedParceiros ? 'SIM' : 'NÃO')

      if (cachedParceiros) {
        let parsedCache
        try {
          parsedCache = JSON.parse(cachedParceiros)
          console.log('📦 Tipo do cache parseado:', Array.isArray(parsedCache) ? 'Array' : typeof parsedCache)
          console.log('📦 Quantidade de itens no cache:', Array.isArray(parsedCache) ? parsedCache.length : 'não é array')

          // Validar se o cache está no formato correto
          if (!Array.isArray(parsedCache) && !parsedCache.parceiros && !parsedCache.data) {
            console.warn('⚠️ Cache em formato inválido, removendo e solicitando novo prefetch')
            sessionStorage.removeItem('cached_parceiros')
            toast.warning('Cache desatualizado. Por favor, faça logout e login novamente para atualizar os dados.')
            setParceiros([])
            setShowParceirosDropdown(false)
            return
          }
          ...
          if (Array.isArray(parsedCache) && parsedCache.length > 0) {
            console.log('📦 Primeiro parceiro do cache:', parsedCache[0])
          }
        } catch (e) {
        ...
        // Lógica de filtragem - suportar diferentes estruturas de cache
        let allParceiros = []

        if (Array.isArray(parsedCache)) {
          allParceiros = parsedCache
        } else if (parsedCache.parceiros && Array.isArray(parsedCache.parceiros)) {
          allParceiros = parsedCache.parceiros
        } else if (parsedCache.data && Array.isArray(parsedCache.data)) {
          allParceiros = parsedCache.data
        } else {
          // Se a estrutura não for reconhecida, tentar extrair todos os valores
          console.warn('⚠️ Estrutura de cache não reconhecida, tentando extrair valores:', Object.keys(parsedCache))
          const possibleArrays = Object.values(parsedCache).filter(v => Array.isArray(v))
          if (possibleArrays.length > 0) {
            allParceiros = possibleArrays[0] as any[]
          }
        }

        console.log(`📊 Total de parceiros disponíveis para filtrar: ${allParceiros.length}`)

        if (allParceiros.length === 0) {
          console.error('❌ Cache parseado mas nenhum parceiro extraído. Estrutura do cache:', parsedCache)
        }

        if (allParceiros.length === 0) {
          console.warn('⚠️ Cache de parceiros existe mas está vazio')
          setParceiros([])
          setShowParceirosDropdown(false)
          return
        }
        ...
        console.log(`✅ ${filtered.length} parceiros filtrados do cache (PedidoFromLead)`)

        if (filtered.length > 0) {
          console.log('📋 Primeiros 3 parceiros filtrados:', filtered.slice(0, 3).map(p => ({
            CODPARC: p.CODPARC,
            NOMEPARC: p.NOMEPARC,
            CGC_CPF: p.CGC_CPF
          })))
      ...
      // Se não houver cache de parceiros, não fazemos a busca na API Sankhya.
      console.warn('⚠️ Cache de parceiros vazio. Nenhuma busca na API Sankhya realizada.')
      console.log('💡 Dica: Verifique se o prefetch foi executado corretamente no login')
      setParceiros([])
      setShowParceirosDropdown(false)

    } catch (error) {
      console.error('❌ Erro ao buscar parceiros no cache:', error)
      setParceiros([])
      setShowParceirosDropdown(false)
    }
  }
  ...
  const selecionarParceiro = async (parceiro: any) => {
    ...
    // Sincronizar com o lead apenas se estiver vinculado
    if (isLeadVinculado && dadosIniciais.CODLEAD) {
      try {
        console.log('🔄 Sincronizando parceiro com o lead:', dadosIniciais.CODLEAD)
        ...
        console.log('✅ Parceiro do lead atualizado no banco')
        toast.success(`Parceiro atualizado!`, {
          description: `${nomeParc} vinculado ao lead`
        })
      } catch (error: any) {
        console.error('❌ Erro ao sincronizar parceiro:', error)
  ...
  const removerItem = async (index: number) => {
    ...
    // Sincronizar com o lead apenas se estiver vinculado
    if (isLeadVinculado && dadosIniciais.CODLEAD && itemParaRemover) {
      try {
    ...
    try {
      ...
      if (cachedEstoques) {
        try {
          ...
          console.warn('⚠️ Erro ao buscar estoque do cache');
        }
      }

      // Buscar preço do cache de exceções
      if (cachedExcecoesPrecos && tabelaSelecionada) {
        try {
          ...
          const excecao = excecoes.find((e: any) =>
            String(e.CODPROD) === String(produto.CODPROD) &&
            String(e.NUTAB) === String(tabelaSelecionada)
          );
          if (excecao && excecao.VLRVENDA) {
            preco = parseFloat(excecao.VLRVENDA);
            console.log('💰 Preço da exceção do cache:', preco);
          }
        } catch (e) {
          console.warn('⚠️ Erro ao buscar preço do cache');
        }
      }
      ...
      console.log('✅ Usando dados do cache - Estoque:', estoqueTotal, 'Preço:', preco);
  ...
  const handleConfirmarProdutoEstoque = async (produto: any, preco: number, quantidade: number) => {
    ...
    const novoItem: ItemPedido = {
      ...
      QTDNEG: quantidade, // Usar quantidade passada como argumento
      VLRUNIT: preco,
      PERCDESC: 0, // Resetar desconto ao adicionar/editar via estoque
      CODLOCALORIG: "700",
      CONTROLE: "007",
      AD_QTDBARRA: 1,
      CODVOL: "UN",
    ...
    if (currentItemIndex !== null) {
      ...
      novoItem.QTDNEG = quantidade // Garante que a quantidade selecionada no modal é usada
      novoItem.PERCDESC = itemExistente.PERCDESC // Preserva o desconto do item existente
      novoItem.CODLOCALORIG = itemExistente.CODLOCALORIG // Preserva o local de origem
      ...
      // Sincronizar edição com o lead apenas se estiver vinculado
      if (isLeadVinculado && dadosIniciais.CODLEAD) {
        try {
          console.log('🔄 Buscando produto do lead para atualizar...');
          console.log('Editando item na posição:', currentItemIndex);
          ...
          console.log('Produtos do lead encontrados:', produtosLead);

          // Buscar pelo índice/posição do produto na lista (mais confiável)
          const produtoLead = produtosLead[currentItemIndex];

          console.log('Produto do lead encontrado (por índice):', produtoLead);

          if (produtoLead?.CODITEM) {
            console.log('🔄 Atualizando produto CODITEM:', produtoLead.CODITEM);
            ...
            console.log('✅ Produto e lead atualizados. Novo total:', result.novoValorTotal);

            toast.success("Produto e lead atualizados!", {
              description: `Novo valor total: R$ ${result.novoValorTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`
            });
          } else {
            console.warn('⚠️ Produto não encontrado no lead, apenas atualizando localmente');
      ...
      if (isLeadVinculado && dadosIniciais.CODLEAD) {
        try {
          console.log('🔄 Adicionando produto ao lead:', dadosIniciais.CODLEAD)
          ...
          if (response.ok) {
            ...
            throw new Error('Erro ao adicionar produto ao lead')
          }
        } catch (error: any) {
          console.error('❌ Erro ao sincronizar produto:', error)
          toast.error('Produto adicionado localmente, mas houve erro ao sincronizar com o lead')
        }
      } else {
  ...
  const salvarPedido = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      console.log('📦 Dados completos sendo enviados para API:', pedidoData)
      ...
      if (isLeadVinculado && dadosIniciais.CODLEAD) {
        ...
        // Pedido independente (sem lead vinculado)
        toast.success("Pedido criado com sucesso!", {
          description: `Pedido: ${resultadoPedido.nunota}`,
          duration: 3000
        })
  ...
  return (
    <div className="space-y-3 md:space-y-4">
                ...
                <div className="space-y-1 md:space-y-2 md:col-span-2">
                  <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <p className="text-xs text-blue-800">
                      <span className="font-semibold">Configuração automática:</span> Os campos Tipo de Operação, Modelo da Nota, Tipo de Movimento e Condição Comercial foram configurados automaticamente pelo Tipo de Pedido selecionado.
                    </p>
                  </div>
                </div>
              )}
      ...
      <ProdutoSelectorModal
        ...
        onConfirm={async (produto: any, preco: number, quantidade: number) => {
          if (currentItemIndex !== null) {
            ...
            const itemExistente = itens[currentItemIndex]
            const novoItem: ItemPedido = {
              ...itemExistente, // Preserva outros campos como PERCDESC, CODLOCALORIG, etc.
            ...
            // Sincronizar produto com o lead apenas se estiver vinculado
            if (isLeadVinculado && dadosIniciais.CODLEAD) {
              try {
                const vlrTotal = preco * quantidade
                const response = await fetch('/api/leads/produtos/adicionar', {
      ...
      <EstoqueModal
        ...
        onClose={() => {
          setShowEstoqueModal(false)
          setShowProdutoModal(true) // Reabrir modal de produto após fechar estoque
        }}
